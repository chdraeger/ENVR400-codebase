# =================================================================================
# Read shapefile/locations and plot on interactive map based on features
# =================================================================================
# content:
# - read shapefile
...
pal <- colorNumeric(palette = c("darkblue", "darkred"),
...
leaflet() %>%
  addTiles() %>%
  addProviderTiles("Esri.WorldTopoMap") %>% # select background
  addCircles(
    data = loc$geometry, # number of plots in each community garden
    radius = loc$number_of_p*2, # multiply by a constant for visibility
